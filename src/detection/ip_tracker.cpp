...
//  DstTracker
// ═══════════════════════════════════════════════════════════════════════════
bool DstTracker::withStats(uint32_t dst_ip,
                            const std::function<void(DstStats&)>& fn) {
    std::lock_guard<std::mutex> lock(mutex_);

    auto it = table_.find(dst_ip);
    if (it == table_.end()) {
        auto [ins, ok] = table_.emplace(dst_ip, DstStats{});
        ins->second.window_start = Clock::now();
        it = ins;
    }

    last_seen_[dst_ip] = Clock::now();
    fn(it->second);
    return true;
}

size_t DstTracker::cleanup(double idle_sec) {
    std::lock_guard<std::mutex> lock(mutex_);
    const auto now = Clock::now();
    size_t removed = 0;
    for (auto it = table_.begin(); it != table_.end(); ) {
