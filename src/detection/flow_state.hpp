...
struct FlowState {
    ...
    TimePoint   http_start;             // Khi nhận byte HTTP đầu tiên
    uint64_t    http_bytes_received  = 0;
    uint32_t    concurrent_conn      = 0; // Số kết nối đồng thời từ src_ip

    // --- Port Scan Detection ---
    std::set<uint16_t> dst_ports_seen;  // Các port đã quét
