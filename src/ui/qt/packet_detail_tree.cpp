...
// ─── Palette ──────────────────────────────────────────────────────────────────
//  BG_TREE      #ffffff    nền tree
//  BG_ALT       #f8f9fd    alternate / branch
//  BORDER       #d0d4e8    viền
//  SECTION_FG   #3355cc    tiêu đề section (bold)
//  FIELD_FG     #1a1a3e    field text chính
//  WARN_FG      #cc6600    cảnh báo truncated
//  THREAT_SEC   #cc2222    section threat
//  THREAT_FLD   #cc4444    field trong threat
//  NOTE_FG      #888899    ghi chú evicted
//  HTTP_DNS_FG  #3355cc    section HTTP/DNS
//  HTTP_FLD     #444466    field HTTP/DNS
//  SELECTED_BG  #dce8ff    item selected
// ─────────────────────────────────────────────────────────────────────────────
    ...
    : QTreeWidget(parent)
{
    ...
    setStyleSheet(
        ...
        "QTreeWidget::item:hover { background: #eef0ff; }"
        "QTreeWidget::item:selected {"
        "  background: #dce8ff; color: #0a0a6e; }"
        "QTreeWidget::branch {"
        "  background: #f8f9fd; }"
        "QTreeWidget::branch:has-siblings:adjoins-item {"
        "  border-image: none; }"
        "QScrollBar:vertical {"
        "  background: #f0f1f8; width: 8px; border: none; }"
        "QScrollBar::handle:vertical {"
        "  background: #b0b8d8; border-radius: 4px; min-height: 20px; }"
...
QTreeWidgetItem* PacketDetailTree::addSection(const QString& title,
    ...
    item->setText(0, summary.isEmpty()
        ? title : title + "  (" + summary + ")");
    item->setForeground(0, QColor("#3355cc"));
    item->setBackground(0, QColor("#f0f4ff"));
    QFont f = item->font(0);
    f.setBold(true);
    item->setFont(0, f);
...
static void addField(QTreeWidgetItem* parent,
                     const QString&   key,
                     const QString&   value,
                     const QColor&    color = QColor("#1a1a3e")) {
...
void PacketDetailTree::showPacket(const PacketInfo& record) {
    ...
    // ── 1. Frame ──────────────────────────────────────────────────────────────
    {
        ...
        if (record.cap_len < record.orig_len)
            addField(frame, "⚠ Truncated",
                    ...
                    QColor("#cc6600"));   // ← warm amber thay #ffaa44
    }
    ...
    if (record.raw_data && !record.raw_data->empty()) {
        ...
        note->setForeground(0, QColor("#888899"));
    }
...
void PacketDetailTree::showEthernet(const uint8_t* data, uint32_t len) {
    ...
    switch (eth_type) {
        case 0x0800:
            if (l3len >= sizeof(struct ip)) showIPv4(l3, l3len);
            break;
        case 0x0806: showARP(l3, l3len);       break;
        case 0x86DD: showIPv6Stub(l3, l3len);  break;
        default: break;
    }
}
...
void PacketDetailTree::showIPv4(const uint8_t* data, uint32_t len) {
    ...
    auto* sec = addSection("🌐 Internet Protocol v4", src + " → " + dst);
    ...
    addField(sec, "Source IP",      src, QColor("#227744"));
    addField(sec, "Destination IP", dst, QColor("#cc2222"));
...
void PacketDetailTree::showTCP(const uint8_t* data, uint32_t len) {
    ...
    addField(sec, "Flags",            flags, QColor("#cc5500"));
...
void PacketDetailTree::showFromFields(const PacketInfo& pkt) {
    ...
    if (pkt.src_ip != 0 || pkt.dst_ip != 0) {
        ...
        auto* ip_sec = addSection("🌐 Internet Protocol v4",
                                   src + " → " + dst);
        addField(ip_sec, "Source IP",      src, QColor("#227744"));
        addField(ip_sec, "Destination IP", dst, QColor("#cc2222"));
        ...
        if (pkt.protocol == IPPROTO_TCP) {
            ...
            addField(tcp_sec, "Source Port",
                     QString::number(pkt.src_port), QColor("#3355cc"));
            addField(tcp_sec, "Destination Port",
                     QString::number(pkt.dst_port), QColor("#3355cc"));
            addField(tcp_sec, "Flags",
                     flagsToString(pkt.tcp_flags), QColor("#cc5500"));
...
void PacketDetailTree::showHTTP(QTreeWidgetItem* parent,
    ...
    while (std::getline(ss, line) && count < 20) {
        ...
        child->setForeground(0, QColor("#444466"));
        ++count;
    }
}
...
void PacketDetailTree::showThreat(const PacketInfo& record) {
    ...
    auto addThreat = [&](const QString& k, const QString& v) {
        ...
        c->setForeground(0, QColor("#cc4444"));
    };

    addThreat("Threat Type", QString::fromStdString(record.threat_type));
    addThreat("Action",      QString::fromStdString(record.action));
}
